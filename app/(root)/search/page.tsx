...
export async function generateMetadata(props: {
  ...
  if (isQuerySet || isPriceSet || isCategorySet || isRatingSet) {
    return {
...
const SearchPage = async (props: {
  ...
  const producst = await getAllProducts({
    query: q,
    category,
    price,
    rating,
    ...
    <div className="grid md:grid-cols-5 md:gap-5">
      ...
      <div className="space-y-4 md:col-span-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {producst.data.length === 0 && <div>No products found</div>}
          {producst.data.map((product) => (
            <ProductCard
              key={product.id}
              product={product}
            />
