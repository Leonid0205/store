...
export async function createPayPalOrder(orderId: string) {
  try {
    ...
    if (order) {
      ...
      throw new Error("Order not found Create Paypal Order");
    }
  } catch (error) {
...
export async function approvePayPalOrder(
  ...
  try {
    ...
    if (!order) {
      throw new Error("Order not found Approve PayPlal Order");
    }
...
async function updateOrderToPaid({
  ...
  if (!order) {
    throw new Error("Order not found Update Order To Paid");
  }
  if (order.isPaid) {
